...
type RoomType = {
  ...
  price: number;
  description: string;
  availableCount: number; // Added this field
};
...
const DetailsBooking = () => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    // --- IMPORTANT: SEND AVAILABLE COUNT ---
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 p-6 sm:p-10 font-sans text-gray-900">
      <div className="max-w-7xl mx-auto">
        ...
        {/* DELETE CONFIRMATION & SUCCESS MODALS (Kept same logic, just hidden for brevity if not changed, but included here for "full code") */}
        <AnimatePresence>
          {deletingId && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full relative z-10 text-center">
                ...
                <div className="flex gap-3 justify-center">
                  <button onClick={() => setDeletingId(null)} className="px-5 py-2.5 rounded-xl text-gray-600 font-semibold hover:bg-gray-100 transition-colors">Cancel</button>
                  <button onClick={confirmDelete} className="px-5 py-2.5 bg-red-600 text-white rounded-xl font-semibold hover:bg-red-700 shadow-lg shadow-red-500/30 transition-all active:scale-95">Delete Room</button>
                </div>
              </motion.div>
            </div>
          )}
        ...
        <AnimatePresence>
          {showDeleteSuccess && (
            <div className="fixed inset-0 z-[70] flex items-center justify-center px-4">
              ...
              <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="bg-white rounded-2xl shadow-2xl p-10 max-w-sm w-full relative z-10">
        ...
        <AnimatePresence>
          {editingRoom && (
            <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => !showUpdateSuccess && setEditingRoom(null)} className="absolute inset-0 bg-black/60 backdrop-blur-sm" />

              <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative z-10">
                <AnimatePresence mode="wait">
                  {showUpdateSuccess ? (
                    <motion.div key="success" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-12">
                    ...
                    <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 sticky top-0 z-20 backdrop-blur-sm">
                        ...
                        <button onClick={() => { setEditingRoom(null); setNewImages(null); }} className="p-2 hover:bg-gray-100 rounded-full text-gray-400 hover:text-gray-600 transition-colors">
                          <X size={20} />
                        </button>
                      </div>
                      ...
                      <form onSubmit={handleUpdate} className="p-8 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <div className="relative">
                              ...
                              <select className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none" value={editingRoom.roomType} onChange={e => setEditingRoom({ ...editingRoom, roomType: e.target.value })}>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="relative">
                              ...
                              <input type="number" className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none" value={editingRoom.price} onChange={e => setEditingRoom({ ...editingRoom, price: Number(e.target.value) })} />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <div className="space-y-2">
                            ...
                            <input type="number" className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none" value={editingRoom.beds} onChange={e => setEditingRoom({ ...editingRoom, beds: Number(e.target.value) })} />
                          </div>
                          <div className="space-y-2">
                            <label className="text-sm font-semibold text-gray-700">Guest Capacity</label>
                            <input type="number" className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none" value={editingRoom.capacity} onChange={e => setEditingRoom({ ...editingRoom, capacity: Number(e.target.value) })} />
                          </div>
                          
                          {/* STOCK INPUT */}
                          <div className="space-y-2">
                            <label className="text-sm font-bold text-blue-600">Available Stock</label>
                            <input 
                                type="number" 
                                min="0" 
                                className="w-full px-4 py-2.5 bg-blue-50 border border-blue-200 text-blue-800 font-bold rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none" 
                                value={editingRoom.availableCount} 
                                onChange={e => setEditingRoom({ ...editingRoom, availableCount: Number(e.target.value) })} 
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            ...
                            <textarea rows={3} className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl resize-none focus:ring-2 focus:ring-blue-500 outline-none" value={editingRoom.description} onChange={e => setEditingRoom({ ...editingRoom, description: e.target.value })} />
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 flex flex-col items-center justify-center hover:bg-gray-50 transition-colors cursor-pointer group relative">
                            <input type="file" multiple accept="image/*" onChange={e => setNewImages(e.target.files)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                            <div className="p-3 bg-blue-50 rounded-full mb-3 group-hover:scale-110 transition-transform"><Upload className="text-blue-500 w-6 h-6" /></div>
                            <p className="text-sm text-gray-600 font-medium">{newImages ? `${newImages.length} files selected` : 'Click to replace existing images'}</p>
                          </div>
                          {!newImages && (
                        ...
                        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                          <button type="button" onClick={() => { setEditingRoom(null); setNewImages(null); }} className="px-5 py-2.5 rounded-xl text-gray-600 font-semibold hover:bg-gray-100 transition-colors">Cancel</button>
                          <button type="submit" disabled={loading} className="px-8 py-2.5 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-600/30 hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-70 disabled:scale-100">{loading ? 'Saving...' : 'Save Changes'}</button>
                        </div>
                      </form>
                    </motion.div>
                  )}
